...
def compute_cluster_scores(metrics: Dict[str, Any]) -> Dict[str, float]:
    ...
    if metrics.get('exposure_totals'):
        clusters['financial_exposure'] += min(CLUSTERS['financial_exposure'], sum(metrics['exposure_totals'].values()) / 1000.0)
